...
RSpec.describe UsersHelper, type: :helper do
  describe '#short_user_name returns short version of user\'s name for navbar' do
    it 'returns only 20 characters of name if the length of name is greater than 20 and adds 3 dots at the end' do
      user = build(:user, name: 'Aurangzaib Danial Liaqat Khan')
      expect(helper.short_user_name(user).length).to eq(23)
      expect(helper.short_user_name(user)[-3..-1]).to eq('...')
    end

    it 'does not add three dots at the end of user\'s name length is less than or equal to 20 ' do
